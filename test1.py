...
def get_pod_name(namespace, app_label, max_retries=5, delay_seconds=2):
    """Dynamically fetches the name of a running pod."""
    cmd = (
    ...
    raise RuntimeError(f"Timeout: No pod found for app '{app_label}'")

def wait_for_config_propagation(namespace, app_label, grep_cmd_arg, max_retries=30, delay_seconds=1):
    """Checks Envoy Config Dump (Control Plane)."""
...
def wait_for_target_app_health(namespace, client_pod_name, target_url, max_retries=30):
    ...
    for _ in range(max_retries):
        ...
        if stdout.strip() == "200":
            ...
            return True
        time.sleep(1)
    raise RuntimeError("Target App failed to become healthy")

# --- NEW FUNCTION ---
def wait_for_header_response(namespace, client_pod_name, target_url, max_retries=30):
    """
    Functional Warm-Up:
    Polls the 503 endpoint until we explicitly see the 'retry-after' header.
    This guarantees the EnvoyFilter is ACTUALLY applying to traffic.
    """
    logging.info("Warming up Header Logic: Polling %s for 'retry-after'...", target_url)
    
    # We grep the headers (-v) for the specific header we expect.
    cmd = (
        f"kubectl exec -n {namespace} {client_pod_name} -- "
        f"curl -s -v -H 'Connection: close' '{target_url}' 2>&1 | grep -i 'retry-after:'"
    )

    for attempt in range(1, max_retries + 1):
        try:
            # If grep finds the header, it returns 0 (Success)
            run_command(cmd, check=True)
            logging.info("Success! 'retry-after' header detected in response.")
            return True
        except Exception:
            # Grep failed (header not found yet)
...
def run_retry_after_header_tests():
    ...
    try:
        ...
        # 1. Wait for Config (Control Plane)
        wait_for_config_propagation(TEST_NAMESPACE, CLIENT_APP_NAME, RETRY_ROUTE_NAME)
        # Note: If using original design, check for the header name or filter name
        ...
        # 2. Wait for App Health (Connectivity)
        wait_for_target_app_health(TEST_NAMESPACE, client_pod_name, f"http://{target_vhost}/status/200")

        # 3. NEW STEP: Wait for Header Logic (Behavior)
        # This closes the race condition. We don't start the timer until the header is PROVEN to exist.
        wait_for_header_response(TEST_NAMESPACE, client_pod_name, f"http://{target_vhost}/status/503")

        # 4. Run the Timer Test
        ...
        try:
            ...
            logging.error("Failed to parse curl output: %s", stdout)
        ...
        if http_code == "503" and min_expected_time <= real_time < max_expected_time:
            logging.info("SUCCESS: Test passed! Time %.2fs is valid.", real_time)
        else:
            logging.error("FAILURE: Retry logic mismatch")
            if real_time < min_expected_time:
                logging.error("Too Fast! %.2fs < %ds", real_time, min_expected_time)
